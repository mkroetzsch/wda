...
class EPTurtleFile(entityprocessor.EntityProcessor):
	...
	def processEntity(self,title,revision,isItem,data):
		...
		if 'datatype' in data:
			...
			if data['datatype'] == 'wikibase-item':
				...
				self.__addPO( "wo:propertyType", "wo:propertyTypeUrl" )
			...
			elif data['datatype'] == 'quantity':
				self.__addPO( "wo:propertyType", "wo:propertyTypeQuantity")
			else:
		...
		for sitekey in data['links'].keys() :
			...
			if isinstance(data['links'][sitekey], dict) and 'name' in data['links'][sitekey].keys():  # New format (dict with 'name' (string) and 'badges' (dict))
				articletitle = data['links'][sitekey]['name'].replace(' ','_').encode('utf-8')
			elif isinstance(data['links'][sitekey], str) or isinstance(data['links'][sitekey], unicode): # Old format (name string)
				# Note: "unicode" only works in Python 2, but "str" subsumes unicode in Python 3;
				# so we should get some prototype-software level compatibility from lazy evaluation
				articletitle = data['links'][sitekey].replace(' ','_').encode('utf-8')
			else:
				logging.log('*** Error: Unsupported type "' + str(type(data['links'][sitekey])) + '" format for "' + sitekey + '" (' + str(data['links'][sitekey]) + ').')
				articletitle = "SITEKEYERROR"
	...
	def __knownTypesReport(self):
		if self.propertyLookupCount > 0:
			...
			for key in sorted(self.propertyTypes.keys()):
				if self.propertyTypes[key] == None: # failed to find type; do not export
					continue
				if count > 0:
					logging.logMore(", ")
				if count % 4 == 0:
	...
	def __fetchPropertyType(self,propertyTitle):
		...
		try:
			...
			logging.log('*** Error: Could not find the datatype for property ' + propertyTitle + ' online. Export might be erroneous.')
		except IOError:
			logging.log('*** Error when trying to fetch property type from wikidata.org.')
	...
	# Encode decimal literals for use in Turtle.
	def __encodeDecimalLiteral(self,number):
		return '"' + str(number) + '"^^x:decimal'
	...
	def __writeStatementData(self,statement):
		...
		self.valuesQT = {} # collect quantity values and export them after each statement
		...
		# Export quantities
		for key in self.valuesQT.keys():
			self.__writeQuantityValue(key,self.valuesQT[key])
	...
	# Write the data for a quantity datavalue with the given local name.
	def __writeQuantityValue(self,localname,value):
		self.__startTriples( 'w:' + localname, "a", "wo:QuantityValue" )
		self.__addPO( "wo:numericValue", self.__encodeDecimalLiteral(value['amount']) )
		self.__addPO( "wo:lowerBound", self.__encodeDecimalLiteral(value['lowerBound']) )
		self.__addPO( "wo:upperBound", self.__encodeDecimalLiteral(value['upperBound']) )
		self.__endTriples()
		...
		if snak[0] == 'value' :
			...
			if self.dataFilter.includePropertyType(datatype):
				if datatype == 'wikibase-item':
					...
					key = 'QT' + self.__getHashForLocalName(snak[3])
					self.valuesQT[key] = snak[3]
					self.__addPO( prop, "w:" + key )
				elif datatype == None :
...
knownPropertyTypes = {
        ...
        'P1017' : 'string', 'P1019' : 'url', 'P102' : 'wikibase-item', 'P1025' : 'string', 
        'P1027' : 'wikibase-item', 'P103' : 'wikibase-item', 'P1030' : 'string', 'P1031' : 'string', 
        'P1033' : 'wikibase-item', 'P1034' : 'wikibase-item', 'P1036' : 'string', 'P1037' : 'wikibase-item', 
        'P1038' : 'wikibase-item', 'P1039' : 'wikibase-item', 'P1040' : 'wikibase-item', 'P1042' : 'string', 
        'P1044' : 'string', 'P1047' : 'string', 'P1048' : 'string', 'P105' : 'wikibase-item', 
        'P1054' : 'string', 'P1055' : 'string', 'P1056' : 'wikibase-item', 'P1058' : 'string', 
        'P1059' : 'string', 'P106' : 'wikibase-item', 'P1065' : 'url', 'P1066' : 'wikibase-item', 
        'P1067' : 'string', 'P1069' : 'string', 'P107' : 'wikibase-item', 'P1070' : 'string', 
        'P1074' : 'wikibase-item', 'P1075' : 'wikibase-item', 'P1076' : 'string', 'P1077' : 'string', 
        'P108' : 'wikibase-item', 'P1080' : 'wikibase-item', 'P1081' : 'quantity', 'P1082' : 'quantity', 
        'P1085' : 'string', 'P1086' : 'quantity', 'P109' : 'commonsMedia', 'P1092' : 'quantity', 
        'P110' : 'wikibase-item', 'P1100' : 'quantity', 'P1101' : 'quantity', 'P1103' : 'quantity', 
        'P1104' : 'quantity', 'P1107' : 'quantity', 'P1108' : 'quantity', 'P1110' : 'quantity', 
        'P1113' : 'quantity', 'P1114' : 'quantity', 'P1115' : 'string', 'P1118' : 'quantity', 
        'P1119' : 'quantity', 'P112' : 'wikibase-item', 'P1120' : 'quantity', 'P1121' : 'quantity', 
        'P1128' : 'quantity', 'P113' : 'wikibase-item', 'P1130' : 'quantity', 'P1132' : 'quantity', 
        'P1134' : 'wikibase-item', 'P114' : 'wikibase-item', 'P1142' : 'wikibase-item', 'P1144' : 'string', 
        'P1146' : 'string', 'P1148' : 'quantity', 'P1149' : 'string', 'P115' : 'wikibase-item', 
        'P1150' : 'string', 'P117' : 'commonsMedia', 'P118' : 'wikibase-item', 'P119' : 'wikibase-item', 
        'P121' : 'wikibase-item', 'P122' : 'wikibase-item', 'P123' : 'wikibase-item', 'P126' : 'wikibase-item', 
        'P127' : 'wikibase-item', 'P131' : 'wikibase-item', 'P132' : 'wikibase-item', 'P133' : 'wikibase-item', 
        ...
        'P167' : 'wikibase-item', 'P168' : 'wikibase-item', 'P169' : 'wikibase-item', 'P17' : 'wikibase-item', 
        'P170' : 'wikibase-item', 'P171' : 'wikibase-item', 'P172' : 'wikibase-item', 'P173' : 'wikibase-item', 
        'P175' : 'wikibase-item', 'P176' : 'wikibase-item', 'P177' : 'wikibase-item', 'P178' : 'wikibase-item', 
        'P179' : 'wikibase-item', 'P18' : 'commonsMedia', 'P180' : 'wikibase-item', 'P181' : 'commonsMedia', 
        'P183' : 'wikibase-item', 'P184' : 'wikibase-item', 'P185' : 'wikibase-item', 'P186' : 'wikibase-item', 
        'P189' : 'wikibase-item', 'P19' : 'wikibase-item', 'P190' : 'wikibase-item', 'P193' : 'wikibase-item', 
        'P194' : 'wikibase-item', 'P195' : 'wikibase-item', 'P196' : 'wikibase-item', 'P197' : 'wikibase-item', 
        'P198' : 'wikibase-item', 'P199' : 'wikibase-item', 'P20' : 'wikibase-item', 'P200' : 'wikibase-item', 
        'P201' : 'wikibase-item', 'P202' : 'wikibase-item', 'P205' : 'wikibase-item', 'P206' : 'wikibase-item', 
        ...
        'P220' : 'string', 'P223' : 'string', 'P225' : 'string', 'P227' : 'string', 
        ...
        'P264' : 'wikibase-item', 'P268' : 'string', 'P269' : 'string', 'P27' : 'wikibase-item', 
        'P270' : 'string', 'P271' : 'string', 'P272' : 'wikibase-item', 'P274' : 'string', 
        'P275' : 'wikibase-item', 'P276' : 'wikibase-item', 'P277' : 'wikibase-item', 'P279' : 'wikibase-item', 
        'P281' : 'string', 'P282' : 'wikibase-item', 'P286' : 'wikibase-item', 'P287' : 'wikibase-item', 
        ...
        'P300' : 'string', 'P301' : 'wikibase-item', 'P304' : 'string', 'P306' : 'wikibase-item', 
        'P31' : 'wikibase-item', 'P344' : 'wikibase-item', 'P345' : 'string', 'P347' : 'string', 
        'P348' : 'string', 'P349' : 'string', 'P35' : 'wikibase-item', 'P350' : 'string', 
        'P352' : 'string', 'P355' : 'wikibase-item', 'P356' : 'string', 'P357' : 'string', 
        ...
        'P395' : 'string', 'P396' : 'string', 'P397' : 'wikibase-item', 'P40' : 'wikibase-item', 
        ...
        'P409' : 'string', 'P41' : 'commonsMedia', 'P410' : 'wikibase-item', 'P412' : 'wikibase-item', 
        'P413' : 'wikibase-item', 'P414' : 'wikibase-item', 'P417' : 'wikibase-item', 'P418' : 'wikibase-item', 
        'P421' : 'wikibase-item', 'P424' : 'string', 'P425' : 'wikibase-item', 'P426' : 'string', 
        ...
        'P462' : 'wikibase-item', 'P463' : 'wikibase-item', 'P465' : 'string', 'P466' : 'wikibase-item', 
        'P467' : 'wikibase-item', 'P47' : 'wikibase-item', 'P473' : 'string', 'P474' : 'string', 
        'P477' : 'string', 'P478' : 'string', 'P480' : 'string', 'P484' : 'string', 
        'P485' : 'wikibase-item', 'P486' : 'string', 'P487' : 'string', 'P488' : 'wikibase-item', 
        'P489' : 'wikibase-item', 'P490' : 'string', 'P492' : 'string', 'P493' : 'string', 
        'P494' : 'string', 'P495' : 'wikibase-item', 'P497' : 'string', 'P498' : 'string', 
        'P50' : 'wikibase-item', 'P500' : 'wikibase-item', 'P501' : 'wikibase-item', 'P504' : 'wikibase-item', 
        'P506' : 'string', 'P508' : 'string', 'P509' : 'wikibase-item', 'P51' : 'commonsMedia', 
        'P511' : 'wikibase-item', 'P512' : 'wikibase-item', 'P513' : 'string', 'P516' : 'wikibase-item', 
        'P518' : 'wikibase-item', 'P520' : 'wikibase-item', 'P521' : 'wikibase-item', 'P522' : 'wikibase-item', 
        'P523' : 'wikibase-item', 'P524' : 'wikibase-item', 'P525' : 'string', 'P527' : 'wikibase-item', 
        'P528' : 'string', 'P529' : 'string', 'P53' : 'wikibase-item', 'P530' : 'wikibase-item', 
        'P531' : 'wikibase-item', 'P532' : 'wikibase-item', 'P535' : 'string', 'P536' : 'string', 
        'P539' : 'string', 'P54' : 'wikibase-item', 'P542' : 'wikibase-item', 'P543' : 'wikibase-item', 
        ...
        'P557' : 'string', 'P558' : 'string', 'P559' : 'wikibase-item', 'P560' : 'wikibase-item', 
        'P561' : 'string', 'P562' : 'wikibase-item', 'P563' : 'string', 'P564' : 'string', 
        'P566' : 'wikibase-item', 'P569' : 'time', 'P57' : 'wikibase-item', 'P570' : 'time', 
        'P571' : 'time', 'P574' : 'time', 'P575' : 'time', 'P576' : 'time', 
        'P577' : 'time', 'P579' : 'wikibase-item', 'P58' : 'wikibase-item', 'P580' : 'time', 
        'P582' : 'time', 'P585' : 'time', 'P586' : 'string', 'P587' : 'string', 
        'P59' : 'wikibase-item', 'P590' : 'string', 'P592' : 'string', 'P597' : 'string', 
        ...
        'P611' : 'wikibase-item', 'P612' : 'wikibase-item', 'P613' : 'string', 'P618' : 'wikibase-item', 
        'P619' : 'time', 'P624' : 'wikibase-item', 'P625' : 'globe-coordinate', 'P627' : 'string', 
        'P629' : 'wikibase-item', 'P630' : 'string', 'P631' : 'wikibase-item', 'P633' : 'string', 
        'P634' : 'wikibase-item', 'P635' : 'string', 'P637' : 'string', 'P638' : 'string', 
        'P640' : 'string', 'P641' : 'wikibase-item', 'P642' : 'wikibase-item', 'P646' : 'string', 
        'P648' : 'string', 'P649' : 'string', 'P65' : 'wikibase-item', 'P653' : 'string', 
        'P655' : 'wikibase-item', 'P657' : 'string', 'P658' : 'wikibase-item', 'P66' : 'wikibase-item', 
        'P661' : 'string', 'P662' : 'string', 'P664' : 'wikibase-item', 'P665' : 'string', 
        'P668' : 'string', 'P669' : 'wikibase-item', 'P670' : 'string', 'P672' : 'string', 
        'P673' : 'string', 'P674' : 'wikibase-item', 'P676' : 'wikibase-item', 'P677' : 'string', 
        'P680' : 'wikibase-item', 'P681' : 'wikibase-item', 'P682' : 'wikibase-item', 'P683' : 'string', 
        'P685' : 'string', 'P686' : 'string', 'P687' : 'string', 'P69' : 'wikibase-item', 
        'P691' : 'string', 'P694' : 'wikibase-item', 'P695' : 'string', 'P697' : 'wikibase-item', 
        'P7' : 'wikibase-item', 'P70' : 'wikibase-item', 'P702' : 'wikibase-item', 'P703' : 'wikibase-item', 
        'P705' : 'string', 'P706' : 'wikibase-item', 'P708' : 'wikibase-item', 'P709' : 'string', 
        'P71' : 'wikibase-item', 'P710' : 'wikibase-item', 'P711' : 'string', 'P712' : 'string', 
        'P713' : 'string', 'P714' : 'string', 'P715' : 'string', 'P716' : 'string', 
        'P718' : 'string', 'P720' : 'wikibase-item', 'P721' : 'string', 'P722' : 'string', 
        'P725' : 'wikibase-item', 'P729' : 'time', 'P734' : 'wikibase-item', 'P735' : 'wikibase-item', 
        'P736' : 'wikibase-item', 'P737' : 'wikibase-item', 'P74' : 'wikibase-item', 'P740' : 'wikibase-item', 
        'P741' : 'wikibase-item', 'P742' : 'string', 'P743' : 'string', 'P744' : 'wikibase-item', 
        'P747' : 'wikibase-item', 'P749' : 'wikibase-item', 'P750' : 'wikibase-item', 'P757' : 'string', 
        'P758' : 'string', 'P759' : 'string', 'P76' : 'wikibase-item', 'P761' : 'string', 
        'P762' : 'string', 'P763' : 'string', 'P764' : 'string', 'P765' : 'wikibase-item', 
        'P766' : 'wikibase-item', 'P767' : 'wikibase-item', 'P768' : 'wikibase-item', 'P77' : 'wikibase-item', 
        'P770' : 'wikibase-item', 'P771' : 'string', 'P772' : 'string', 'P773' : 'string', 
        'P774' : 'string', 'P775' : 'string', 'P78' : 'wikibase-item', 'P780' : 'wikibase-item', 
        'P782' : 'string', 'P790' : 'wikibase-item', 'P791' : 'string', 'P792' : 'string', 
        'P793' : 'wikibase-item', 'P794' : 'wikibase-item', 'P799' : 'string', 'P800' : 'wikibase-item', 
        'P802' : 'wikibase-item', 'P803' : 'wikibase-item', 'P804' : 'string', 'P805' : 'wikibase-item', 
        'P806' : 'string', 'P808' : 'string', 'P809' : 'string', 'P81' : 'wikibase-item', 
        'P813' : 'time', 'P814' : 'wikibase-item', 'P815' : 'string', 'P816' : 'wikibase-item', 
        'P817' : 'wikibase-item', 'P827' : 'string', 'P828' : 'wikibase-item', 'P829' : 'string', 
        'P830' : 'string', 'P831' : 'wikibase-item', 'P832' : 'wikibase-item', 'P833' : 'wikibase-item', 
        'P836' : 'string', 'P837' : 'wikibase-item', 'P838' : 'string', 'P84' : 'wikibase-item', 
        'P840' : 'wikibase-item', 'P841' : 'wikibase-item', 'P842' : 'string', 'P846' : 'string', 
        'P849' : 'string', 'P85' : 'wikibase-item', 'P850' : 'string', 'P853' : 'wikibase-item', 
        'P854' : 'url', 'P856' : 'url', 'P858' : 'string', 'P86' : 'wikibase-item', 
        'P862' : 'string', 'P865' : 'string', 'P866' : 'string', 'P867' : 'string', 
        'P868' : 'wikibase-item', 'P87' : 'wikibase-item', 'P872' : 'wikibase-item', 'P878' : 'wikibase-item', 
        'P879' : 'string', 'P88' : 'wikibase-item', 'P882' : 'string', 'P883' : 'string', 
        'P884' : 'string', 'P888' : 'string', 'P898' : 'string', 'P9' : 'wikibase-item', 
        'P901' : 'string', 'P902' : 'string', 'P905' : 'string', 'P906' : 'string', 
        ...
        'P916' : 'wikibase-item', 'P92' : 'wikibase-item', 'P921' : 'wikibase-item', 'P931' : 'wikibase-item', 
        'P933' : 'string', 'P935' : 'string', 'P937' : 'wikibase-item', 'P94' : 'commonsMedia', 
        'P941' : 'wikibase-item', 'P944' : 'wikibase-item', 'P945' : 'wikibase-item', 'P946' : 'string', 
        'P947' : 'string', 'P948' : 'commonsMedia', 'P949' : 'string', 'P950' : 'string', 
        'P951' : 'string', 'P954' : 'string', 'P957' : 'string', 'P958' : 'string', 
        'P959' : 'string', 'P960' : 'string', 'P961' : 'string', 'P963' : 'url', 
        'P964' : 'string', 'P965' : 'string', 'P966' : 'string', 'P969' : 'string', 
        'P97' : 'wikibase-item', 'P971' : 'wikibase-item', 'P972' : 'wikibase-item', 'P973' : 'url', 
        'P98' : 'wikibase-item', 'P982' : 'string', 'P984' : 'string', 'P990' : 'commonsMedia', 
        'P991' : 'wikibase-item', 'P996' : 'commonsMedia', 'P998' : 'string'
}
...
#knownPropertyTypes = {
        ...
        #'P1013' : 'wikibase-item', 'P1014' : 'string', 'P1015' : 'string', 'P1016' : 'wikibase-item',
        #'P1017' : 'string', 'P1018' : 'wikibase-item', 'P1019' : 'url', 'P102' : 'wikibase-item',
        #'P1021' : 'string', 'P1022' : 'string', 'P1023' : 'string', 'P1024' : 'string',
        #'P1025' : 'string', 'P1026' : 'wikibase-item', 'P1027' : 'wikibase-item', 'P1028' : 'wikibase-item',
        #'P1029' : 'wikibase-item', 'P103' : 'wikibase-item', 'P1030' : 'string', 'P1031' : 'string',
        #'P1032' : 'wikibase-item', 'P1033' : 'wikibase-item', 'P1034' : 'wikibase-item', 'P1035' : 'wikibase-item',
        #'P1036' : 'string', 'P1037' : 'wikibase-item', 'P1038' : 'wikibase-item', 'P1039' : 'wikibase-item',
        #'P1040' : 'wikibase-item', 'P1041' : 'wikibase-item', 'P1042' : 'string', 'P1043' : 'string',
        #'P1044' : 'string', 'P1045' : 'string', 'P1046' : 'wikibase-item', 'P1047' : 'string',
        #'P1048' : 'string', 'P1049' : 'wikibase-item', 'P105' : 'wikibase-item', 'P1050' : 'wikibase-item',
        #'P1051' : 'string', 'P1052' : 'string', 'P106' : 'wikibase-item', 'P107' : 'wikibase-item',
        ...
        #'P163' : 'wikibase-item', 'P166' : 'wikibase-item', 'P167' : 'wikibase-item', 'P168' : 'wikibase-item',
        ...
        #'P476' : 'string', 'P477' : 'string', 'P478' : 'string', 'P480' : 'string',
        #'P481' : 'string', 'P483' : 'wikibase-item', 'P484' : 'string', 'P485' : 'wikibase-item',
        #'P486' : 'string', 'P487' : 'string', 'P488' : 'wikibase-item', 'P489' : 'wikibase-item',
        #'P490' : 'string', 'P491' : 'commonsMedia', 'P492' : 'string', 'P493' : 'string',
        #'P494' : 'string', 'P495' : 'wikibase-item', 'P496' : 'string', 'P497' : 'string',
        #'P498' : 'string', 'P50' : 'wikibase-item', 'P500' : 'wikibase-item', 'P501' : 'wikibase-item',
        #'P502' : 'string', 'P503' : 'string', 'P504' : 'wikibase-item', 'P505' : 'wikibase-item',
        #'P506' : 'string', 'P507' : 'string', 'P508' : 'string', 'P509' : 'wikibase-item',
        #'P51' : 'commonsMedia', 'P511' : 'wikibase-item', 'P512' : 'wikibase-item', 'P513' : 'string',
        #'P514' : 'wikibase-item', 'P515' : 'wikibase-item', 'P516' : 'wikibase-item', 'P517' : 'wikibase-item',
        #'P518' : 'wikibase-item', 'P520' : 'wikibase-item', 'P521' : 'wikibase-item', 'P522' : 'wikibase-item',
        #'P523' : 'wikibase-item', 'P524' : 'wikibase-item', 'P525' : 'string', 'P527' : 'wikibase-item',
        #'P528' : 'string', 'P529' : 'string', 'P53' : 'wikibase-item', 'P530' : 'wikibase-item',
        #'P531' : 'wikibase-item', 'P532' : 'wikibase-item', 'P533' : 'wikibase-item', 'P534' : 'wikibase-item',
        #'P535' : 'string', 'P536' : 'string', 'P537' : 'wikibase-item', 'P538' : 'wikibase-item',
        #'P539' : 'string', 'P54' : 'wikibase-item', 'P541' : 'wikibase-item', 'P542' : 'wikibase-item',
        #'P543' : 'wikibase-item', 'P545' : 'wikibase-item', 'P547' : 'wikibase-item', 'P548' : 'wikibase-item',
        #'P549' : 'string', 'P550' : 'wikibase-item', 'P551' : 'wikibase-item', 'P552' : 'wikibase-item',
        #'P553' : 'wikibase-item', 'P554' : 'string', 'P555' : 'string', 'P556' : 'wikibase-item',
        #'P557' : 'string', 'P558' : 'string', 'P559' : 'wikibase-item', 'P560' : 'wikibase-item',
        #'P561' : 'string', 'P562' : 'wikibase-item', 'P563' : 'string', 'P564' : 'string',
        #'P565' : 'wikibase-item', 'P566' : 'wikibase-item', 'P567' : 'wikibase-item', 'P568' : 'wikibase-item',
        #'P569' : 'time', 'P57' : 'wikibase-item', 'P570' : 'time', 'P571' : 'time',
        #'P574' : 'time', 'P575' : 'time', 'P576' : 'time', 'P577' : 'time',
        ...
        #'P678' : 'wikibase-item', 'P680' : 'wikibase-item', 'P681' : 'wikibase-item', 'P682' : 'wikibase-item',
        ...
        #'P690' : 'wikibase-item', 'P691' : 'string', 'P693' : 'wikibase-item', 'P694' : 'wikibase-item',
        ...
        #'P716' : 'string', 'P717' : 'string', 'P718' : 'string', 'P720' : 'wikibase-item',
        #'P721' : 'string', 'P722' : 'string', 'P723' : 'string', 'P724' : 'string',
        #'P725' : 'wikibase-item', 'P726' : 'wikibase-item', 'P727' : 'string', 'P728' : 'string',
        #'P729' : 'time', 'P730' : 'time', 'P731' : 'string', 'P732' : 'string',
        #'P733' : 'string', 'P734' : 'wikibase-item', 'P735' : 'wikibase-item', 'P736' : 'wikibase-item',
        #'P737' : 'wikibase-item', 'P738' : 'wikibase-item', 'P739' : 'wikibase-item', 'P74' : 'wikibase-item',
        #'P740' : 'wikibase-item', 'P741' : 'wikibase-item', 'P742' : 'string', 'P743' : 'string',
        #'P744' : 'wikibase-item', 'P745' : 'string', 'P746' : 'time', 'P747' : 'wikibase-item',
        #'P748' : 'wikibase-item', 'P749' : 'wikibase-item', 'P75' : 'wikibase-item', 'P750' : 'wikibase-item',
        #'P751' : 'wikibase-item', 'P756' : 'wikibase-item', 'P757' : 'string', 'P758' : 'string',
        #'P759' : 'string', 'P76' : 'wikibase-item', 'P761' : 'string', 'P762' : 'string',
        #'P763' : 'string', 'P764' : 'string', 'P765' : 'wikibase-item', 'P766' : 'wikibase-item',
        #'P767' : 'wikibase-item', 'P768' : 'wikibase-item', 'P769' : 'wikibase-item', 'P77' : 'wikibase-item',
        #'P770' : 'wikibase-item', 'P771' : 'string', 'P772' : 'string', 'P773' : 'string',
        #'P774' : 'string', 'P775' : 'string', 'P776' : 'string', 'P777' : 'string',
        #'P778' : 'string', 'P779' : 'string', 'P78' : 'wikibase-item', 'P780' : 'wikibase-item',
        #'P781' : 'string', 'P782' : 'string', 'P784' : 'wikibase-item', 'P785' : 'wikibase-item',
        ...
        #'P794' : 'wikibase-item', 'P795' : 'wikibase-item', 'P796' : 'wikibase-item', 'P798' : 'string',
        ...
        #'P808' : 'string', 'P809' : 'string', 'P81' : 'wikibase-item', 'P812' : 'wikibase-item',
        #'P813' : 'time', 'P814' : 'wikibase-item', 'P815' : 'string', 'P816' : 'wikibase-item',
        #'P817' : 'wikibase-item', 'P818' : 'string', 'P821' : 'string', 'P822' : 'wikibase-item',
        ...
        #'P842' : 'string', 'P843' : 'string', 'P845' : 'string', 'P846' : 'string',
        ...
        #'P863' : 'string', 'P865' : 'string', 'P866' : 'string', 'P867' : 'string',
        #'P868' : 'wikibase-item', 'P87' : 'wikibase-item', 'P870' : 'wikibase-item', 'P872' : 'wikibase-item',
        #'P873' : 'wikibase-item', 'P874' : 'string', 'P876' : 'string', 'P877' : 'string',
        #'P878' : 'wikibase-item', 'P879' : 'string', 'P88' : 'wikibase-item', 'P880' : 'wikibase-item',
        #'P881' : 'wikibase-item', 'P882' : 'string', 'P883' : 'string', 'P884' : 'string',
        #'P885' : 'wikibase-item', 'P886' : 'string', 'P888' : 'string', 'P889' : 'string',
        #'P89' : 'wikibase-item', 'P892' : 'string', 'P894' : 'string', 'P897' : 'string',
        ...
        #'P92' : 'wikibase-item', 'P920' : 'string', 'P921' : 'wikibase-item', 'P923' : 'wikibase-item',
        #'P924' : 'wikibase-item', 'P929' : 'wikibase-item', 'P930' : 'wikibase-item', 'P931' : 'wikibase-item',
        #'P933' : 'string', 'P935' : 'string', 'P937' : 'wikibase-item', 'P938' : 'string',
        #'P939' : 'string', 'P94' : 'commonsMedia', 'P940' : 'string', 'P941' : 'wikibase-item',
        #'P942' : 'wikibase-item', 'P943' : 'wikibase-item', 'P944' : 'wikibase-item', 'P945' : 'wikibase-item',
        #'P946' : 'string', 'P947' : 'string', 'P948' : 'commonsMedia', 'P949' : 'string',
        #'P950' : 'string', 'P951' : 'string', 'P952' : 'string', 'P953' : 'url',
        #'P954' : 'string', 'P957' : 'string', 'P958' : 'string', 'P959' : 'string',
        #'P960' : 'string', 'P961' : 'string', 'P962' : 'string', 'P963' : 'url',
        #'P964' : 'string', 'P965' : 'string', 'P966' : 'string', 'P967' : 'wikibase-item',
        #'P968' : 'url', 'P969' : 'string', 'P97' : 'wikibase-item', 'P970' : 'wikibase-item',
        #'P971' : 'wikibase-item', 'P972' : 'wikibase-item', 'P973' : 'url', 'P974' : 'wikibase-item',
        #'P98' : 'wikibase-item', 'P981' : 'string', 'P982' : 'string', 'P984' : 'string',
        #'P988' : 'string', 'P989' : 'commonsMedia', 'P990' : 'commonsMedia', 'P991' : 'wikibase-item',
        #'P992' : 'wikibase-item', 'P993' : 'string', 'P994' : 'string', 'P995' : 'string',
        #'P996' : 'commonsMedia', 'P998' : 'string', 'P999' : 'string'
#}